...
class MasterProfessionalOrganizationOrAssociationCategoryModel extends CrudFormModel{
    ...
    void approve() { 
        if ( MsgBox.confirm('You are about to approve this information. Proceed?')) { 
            getPersistenceService().update([ 
               _schemaname: 'master_tblprofessionalorganizationorassociationcategory', 
               objid : entity.objid, 
               state : 'APPROVED' 
            ]); 
            loadData(); 
