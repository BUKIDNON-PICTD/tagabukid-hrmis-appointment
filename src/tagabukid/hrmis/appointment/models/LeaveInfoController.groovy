...
//        binding.refresh("subform");
        def handlers = Inv.lookupOpeners("leave:section:"+action,[parententity:entity,svc:svc]);
    ...
    void loadSections(action) {
        sections = InvokerUtil.lookupOpeners( "leave:section:"+action,[parententity:entity,svc:svc]).findAll {
            def vw = it.properties.visibleWhen;
            return  ((!vw)  ||  ExpressionResolver.getInstance().evalBoolean( vw, [parententity:entity,svc:svc] ));
        }
        if( sections.size()>0 ) {
