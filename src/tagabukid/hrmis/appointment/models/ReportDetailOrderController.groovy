...
class ReportDetailOrderController extends ReportController {
    
    @Service("TagabukidHRMISDetailOrderReportService")
    def svc;

    String title = 'Detail Order Inventory'
        String REPORT_PATH = 'tagabukid/hrmis/appointment/reports/'
        String reportName = REPORT_PATH + 'detailorder_inventory.jasper';
        def data 
        def entity
        def nowd
    ...
    SubReport[] getSubReports() {
        return [ 
           // new SubReport("INVENTORY", REPORT_PATH + "detailorder_inventory_items.jasper"), 
//
